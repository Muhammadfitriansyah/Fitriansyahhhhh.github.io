    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if (isset($_FILES['file']) && $_FILES['file']['size'] > 0) {
            ...
            if (move_uploaded_file($file['tmp_name'], $filePath)) {
                $sql = "UPDATE orders SET name = '$name', quantity = '$quantity', file_path = '$filePath' WHERE id = $id";
            } else {
...
<body style="font-family: Arial, sans-serif; background-color: #f9f9f9; padding: 20px;">
    ...
    <div style="max-width: 600px; margin: 0 auto; background-color: white; padding: 20px; border-radius: 10px; box-shadow: 0px 0px 10px rgba(0, 0, 0, 0.1);">
        ...
        <form action="" method="POST">
            ...
            <div style="margin-bottom: 15px;">
                <label for="file" style="display: block; margin-bottom: 5px;">Upload New File:</label>
                <input type="file" id="file" name="file" accept=".jpg,.jpeg,.png,.pdf" style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 5px;">
            </div>
            <button type="submit"
